...
class VideoCommentSheet extends StatefulWidget {
  ...
  const VideoCommentSheet({super.key, required this.videoId});
  ...
  static Future<void> show(BuildContext context, String videoId) {
    return showModalBottomSheet(
      ...
      builder: (_) => VideoCommentSheet(videoId: videoId),
    );
  }
...
class _VideoCommentSheetState extends State<VideoCommentSheet> {
  ...
  Future<void> _confirmDelete(BuildContext context, Comment comment) async {
    ...
    if (confirmed == true && context.mounted) {
      await context.read<VideoProvider>().deleteComment(videoId: widget.videoId, commentId: comment.id);
  ...
  Future<void> _submit() async {
    ...
    if (text.isEmpty || text.length > _kMaxCommentLength) return;
    final provider = context.read<VideoProvider>();

    if (_mode == _ComposeMode.edit && _target != null) {
      await provider.editComment(videoId: widget.videoId, commentId: _target!.id, newText: text);
    } else {
  ...
  Widget build(BuildContext context) {
    ...
    final overLimit = _controller.text.length > _kMaxCommentLength;
              ...
              children: [
                ...
                Consumer<VideoProvider>(
                  builder: (context, provider, _) {
                    final video = provider.videoById(widget.videoId);
                    final count = video?.commentCount ?? 0;
                    return Padding(
                ...
                Expanded(
                  child: Consumer<VideoProvider>(
                    builder: (context, provider, _) {
                      final video = provider.videoById(widget.videoId);
                      final topLevel = video?.topLevelComments ?? const <Comment>[];
                      ...
                      return ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final replies = video!.repliesTo(comment.id);
                          return Column(
                ...
                if (overLimit)
                  Padding(
                    ...
                    child: Text('${_controller.text.length}/$_kMaxCommentLength — too long',
